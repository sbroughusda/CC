...
# Import from the enhanced comment builder script (cb3) instead of the original
import cb3
from cb3 import get_documents_for_docket, get_comments_for_document, get_comment_details, save_comments_to_csv
...
app.config['MAX_COMMENTS'] = 500  # Set maximum comments to 500
...
def fetch_comments():
    ...
    max_comments = app.config['MAX_COMMENTS']
    ...
    try:
        ...
        # Get comments for each document, with a limit of 500 total
        all_comments = []
        ...
        comments_message = f'Successfully retrieved {len(all_comments)} comments'
        if len(all_comments) >= max_comments:
            comments_message += f' (limited to maximum of {max_comments})'
        comments_message += f' for docket ID: {docket_id}'
        
        flash(comments_message, 'success')
