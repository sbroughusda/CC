...
#API_KEY_BAK = "lJ1THACguVQVwkfVh1OFjkmhYruFIctj7ncjlxld"
...
def get_documents_for_docket(docket_id):
    ...
    while True:
        ...
        if response.status_code != 200:
            print(f"Error retrieving documents: {response.status_code}")
            print(response.text)
            break
...
def get_comment_details(comment_id):
    ...
    # Try up to 3 times in case of API issues
    for attempt in range(3):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Error retrieving comment details: {response.status_code}")
                time.sleep(1)  # Brief pause before retry
        except Exception as e:
            print(f"Exception retrieving comment: {str(e)}")
            time.sleep(1)  # Brief pause before retry
